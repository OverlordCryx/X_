...
task.spawn(function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/OverlordCryx/X_/refs/heads/main/SLS/DC"))()
end)
task.spawn(function()
...
local function handleFootball(hrp)
    ...
    if teamPos ~= "GK" then
		...
		local function xxhandleFootball(hrp)
    local football = Workspace:FindFirstChild("Misc") and Workspace.Misc:FindFirstChild("Football")
    if not football then return end
    local team = LocalPlayer.Team
    if not team then return end
    ...
    Callback = function(state)
        ...
        conn = RunService.Stepped:Connect(function()
            ...
            for i = 1, dubValue do  
                ...
                handleFootball(hrp)
				xhandleFootball(hrp)
				xxhandleFootball(hrp)
				xxxhandleFootball(hrp)
            end
        end)
    end
})
